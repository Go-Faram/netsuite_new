        ...
        criteria.each_pair do |condition_category, conditions|
          criteria_structure["#{namespace}:#{condition_category}"] = conditions.inject({}) do |h, condition|
            ...
            when 'customFieldList'
              ...
              if NetSuiteNew::Configuration.api_version < "2013_2"
      ...
      def soap_header_extra_info
        ...
        (@options.delete(:preferences) || {})
          .inject({'platformMsgs:SearchPreferences' => {}}) do |h, (k, v)|
            h['platformMsgs:SearchPreferences'][NetSuiteNew::Utilities::Strings.lower_camelcase(k.to_s)] = v
            h
          end
      end
      ...
      module Support
        ...
        module ClassMethods
          def search(options = { }, credentials={})
            response = NetSuiteNew::Actions::Search.call([self, options], credentials)

            if response.success?
              NetSuiteNew::Support::SearchResult.new(response, self, credentials)
            else
              false
            end
          end
