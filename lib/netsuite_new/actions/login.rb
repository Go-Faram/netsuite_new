    ...
    class Login
      ...
      def self.call(credentials)
        ...
        if !credentials[:application_id].nil? && !credentials[:application_id].empty?
          soap_header = NetSuiteNew::Configuration.soap_header.dup
        ...
        passport = NetSuiteNew::Configuration.auth_header.dup
        ...
        begin
          response = NetSuiteNew::Configuration.connection(soap_header: soap_header).call :login, message: passport
        rescue Savon::SOAPFault => e
          error_details = e.to_hash[:fault]

          if error_details[:detail].has_key?(:invalid_credentials_fault)
            return NetSuiteNew::Response.new(
              success: false,
              errors: [ NetSuiteNew::Error.new(
