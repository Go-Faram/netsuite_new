    ...
    module Records
      ...
      def to_record
        attributes.reject { |k,v| self.class.read_only_fields.include?(k) || self.class.search_only_fields.include?(k) }.inject({}) do |hash, (k,v)|
          kname = "#{v.is_a?(NetSuiteNew::Records::NullFieldList) ? v.record_namespace : record_namespace}:"
          kname += k == :klass ? 'class' : NetSuiteNew::Utilities::Strings.lower_camelcase(k.to_s)
      ...
      def to_attributes!(hash, kname, v)
        ...
        if v.kind_of?(NetSuiteNew::Records::RecordRef) && v.type
          hash[:attributes!] ||= {}
          hash[:attributes!][kname] ||= {}
          hash[:attributes!][kname]['type'] = NetSuiteNew::Utilities::Strings.lower_camelcase(v.type)
        end

        if v.kind_of?(NetSuiteNew::Records::CustomRecordRef) && v.type_id
          hash[:attributes!] ||= {}
          hash[:attributes!][kname] ||= {}
          hash[:attributes!][kname]['typeId'] = NetSuiteNew::Utilities::Strings.lower_camelcase(v.type_id)
        end
      end
      ...
      def self.netsuite_type(obj)
        NetSuiteNew::Utilities::Strings.lower_camelcase(record_type_without_namespace(obj))
      end
